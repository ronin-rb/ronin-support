  ...
  module Support
    module Network
      ...
      module SSL
        ...
        # @since 1.1.0
        #
        def self.open?(host,port, bind_host: nil,
                                  bind_port: nil,
                                  timeout: 5,
                                  **kwargs)
          Timeout.timeout(timeout) do
            connect(host,port, bind_host: bind_host,
                               bind_port: bind_port,
                               **kwargs)
          end
        ...
        # @since 1.1.0
        #
        def self.connect_and_send(data,host,port, bind_host: nil,
                                                 bind_port: nil,
                                                 **kwargs)
          socket = connect(host,port, bind_host: bind_host,
                                      bind_port: bind_port,
                                      **kwargs)
          socket.write(data)
        ...
        # @since 1.1.0
        #
        def self.banner(host,port, bind_host: nil, bind_port: nil, **kwargs)
          banner = nil
        ...
        # @since 1.1.0
        #
        def self.send(data,host,port, bind_host: nil, bind_port: nil,**kwargs)
          connect(host,port, bind_host: bind_host,
                             bind_port: bind_port,
                             **kwargs) do |socket|
            socket.write(data)
