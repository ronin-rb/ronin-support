...
describe Ronin::Support::Binary::CTypes::ArrayObjectType do
  let(:type_endian)      { :little }
  let(:type_pack_string) { 'L<'    }

  let(:type) do
    Ronin::Support::Binary::CTypes::UInt32Type.new(
      endian:      type_endian,
      pack_string: type_pack_string
    )
  end
  let(:length) { 10 }
  let(:array_type) { Ronin::Support::Binary::CTypes::ArrayType.new(type,length) }
