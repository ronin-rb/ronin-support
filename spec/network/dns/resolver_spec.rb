...
describe Ronin::Support::Network::DNS::Resolver do
  ...
  describe "#get_ip_address" do
    context "integration", :network do
      ...
      it "must return the first IPv4 or IPv6 address" do
        expect(subject.get_ip_address(hostname)).to eq(ipv4_address)
      end
      ...
      context "when the host name does not have any A or AAAA records" do
        ...
        it "must return nil" do
          expect(subject.get_ip_address(hostname)).to be(nil)
        end
      end
    end
  end
